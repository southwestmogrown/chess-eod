...
const { Pawn, Piece } = require("../pieces");
...
describe("The Pawn class", () => {
  ...
  describe("the canMove() method", () => {
    ...
    context("a piece of the opposing color is in a diagonal square", () => {
      ...
      it("should not allow a pawn to capture a piece directly forward", () => {
        const whitePawn = new Pawn(true);
        const blackPawn = new Pawn(false);

        const testB = b.generateTestBoard();
        testB[6][3] = new Square(6, 3, whitePawn);
        testB[5][3] = new Square(5, 3, blackPawn);

        expect(whitePawn.canMove(testB, testB[6][3], testB[5][3])).to.be.false;

        const whitePawn2 = new Pawn(true);
        const blackPawn2 = new Pawn(false);
        testB[1][4] = new Square(1, 4, blackPawn2);
        testB[2][4] = new Square(2, 4, whitePawn2);

        expect(blackPawn2.canMove(testB, testB[1][4], testB[2][4])).to.be.false;
      });

      it("should not allow a pawn to move two spaces if blocked on first move", () => {
        const whitePawn = new Pawn(true);
        const blockingPawn = new Pawn(false);

        const testB = b.generateTestBoard();
        testB[6][2] = new Square(6, 2, whitePawn);
        testB[5][2] = new Square(5, 2, blockingPawn);

        expect(whitePawn.canMove(testB, testB[6][2], testB[4][2])).to.be.false;

        const blackPawn = new Pawn(false);
        const blockingWhitePawn = new Pawn(true);
        testB[1][5] = new Square(1, 5, blackPawn);
        testB[2][5] = new Square(2, 5, blockingWhitePawn);

        expect(blackPawn.canMove(testB, testB[1][5], testB[3][5])).to.be.false;
      });
    });
  });
